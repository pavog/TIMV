			...
			if (Lobby.started && !Game.started) {
				if (DatabaseHandler.hasPass(player)) {
					if (!Lobby.forcedTraitors.contains(player) && !Lobby.forcedDetectives.contains(player)) {
						if (!(Lobby.forcedDetectives.size() >= TIMV.config.getInt("MaxDetectives"))) {
							Lobby.forcedDetectives.add(player);
							player.sendMessage(MessagesHandler.convert("Detective.Success"));
						} else {
							player.sendMessage(MessagesHandler.convert("Detective.TooMany"));
						}
					} else {
						player.sendMessage(MessagesHandler.convert("Detective.AlreadyMatched"));
					}
				} else {
					player.sendMessage(MessagesHandler.convert("NoPasses"));
				}
			} else {
				player.sendMessage(MessagesHandler.convert("Detective.OnlyInLobby"));
			}
		} else {
